    ...
    def get_similar_products(self, product_id, top_n=3):
        ...
        for i, score in sim_scores[1:]:
            if len(similar_product_indices) < top_n:
                if i < len(self.products_df.index):
                     similar_product_indices.append(i)
            else:
                break
        ...
        similar_product_ids = self.products_df.index[similar_product_indices].tolist()
        return self.products_df.loc[similar_product_ids]


# --- 3. Shopping Bot Logic ---
class ShoppingBot:
    def __init__(self, products_df, recommender):
    ...
    def suggest_complementary_products(self, product_id):
        if product_id not in self.products_df.index:
            # print("Invalid product ID for complementary suggestions.") # Already handled by main loop
            return
